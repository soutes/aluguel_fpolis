...
class IbagyAjaxSpider(scrapy.Spider):
    name = "piramides_ajax"  # Valor original; retiraremos o sufixo "_ajax" para a imobiliária.
    allowed_domains = ["piramides.com.br"]
    start_urls = ["https://piramides.com.br/aluguel/residencial/"]
    ...
    def parse(self, response):
        ...
        if total > results_per_page:
            ...
            # Usamos o endpoint https://piramides.com.br/u-sr.php?queryData=&reload=0 com o parâmetro offset
            for page in range(2, num_pages + 1):
                offset = (page - 1) * results_per_page
                ajax_url = f"https://piramides.com.br/u-sr.php?queryData=&reload=0&offset={offset}"
    ...
    def extract_imovel(self, imovel, response):
        ...
        # Extrai o bairro a partir do endereço (padrão "Rua Nome, Número, Bairro - Cidade/Estado")
        bairro = ""
        if endereco_completo:
            partes = endereco_completo.split(',')
            if len(partes) >= 3:
                sub = partes[2].strip()
                bairro = sub.split(" - ")[0].strip()
