...
class IbagyAjaxSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # Chama AJAX para as demais páginas
        results_per_page = 12
        if total > results_per_page:
            ...
            for page in range(2, num_pages + 1):
                ...
                ajax_url = f"https://ibagy.com.br/u-sr.php?queryData=&reload=0&offset={offset}"
    ...
    def extract_imovel(self, imovel, response):
        # Captura o código do imóvel (data-codigo)
        codigo = imovel.css("button.btn-favorito::attr(data-codigo)").get(default="").strip()
        ...
        endereco_completo = imovel.css(
            'h3[itemprop="streetAddress"]::text'
        ).get(default='').strip()

        # Extrai o bairro (padrão "Rua Nome, Número, Bairro - Cidade/Estado")
        bairro = ""
